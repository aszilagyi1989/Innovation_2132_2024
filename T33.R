...
result_T33_DT <- rbind(Aggregate_INN, Aggregate_NINN)
# result_T33_DT %>% group_by(DATAFLOW, FREQ, TIME_PERIOD, REF_AREA, TABLENAME, ACTIVITY, NUMBER_EMPL, INN_PF, INDICATOR, CIS_INDICATOR, UNIT_MEASURE, UNIT_MULT, DECIMALS) %>% summarise(OBS_VALUE = sum(OBS_VALUE)) -> result_T33_DT
...
colnames(result_T33_DT)[14] <- "OBS_VALUE"
...
result_T33_DT_FINAL <- subset(result_T33_DT_FINAL, TYPE_ENT == "_T")
...
write.table(rbind(result_T33_DT_FINAL, Aggregate_INN, Aggregate_NINN), T33_SDMX_FINAL_RESULT, sep = ";", quote = FALSE, row.names = FALSE, append = FALSE)

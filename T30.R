...
result_T30_DT <- rbind(Aggregate_INN, Aggregate_NINN)
# result_T30_DT %>% group_by(DATAFLOW, FREQ, TIME_PERIOD, REF_AREA, TABLENAME, ACTIVITY, NUMBER_EMPL, INN_PF, INDICATOR, CIS_INDICATOR, UNIT_MEASURE, UNIT_MULT, DECIMALS) %>% summarise(OBS_VALUE = sum(OBS_VALUE)) -> result_T30_DT
...
colnames(result_T30_DT)[14] <- "OBS_VALUE"
...
result_T30_DT_FINAL <- subset(result_T30_DT_FINAL, TYPE_ENT == "_T")
result_T30_DT_FINAL[result_T30_DT_FINAL$TYPE_ENT == "_T", "CONF_STATUS"] <- "N"
...
write.table(rbind(result_T30_DT_FINAL, Aggregate_INN, Aggregate_NINN), T30_SDMX_FINAL_RESULT, sep = ";", quote = FALSE, row.names = FALSE, append = FALSE)

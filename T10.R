...
for(num in 1:length(expression)){
  ...
  if(nrow(INN_DT) != 0){
    ...
    INN_DT %>% mutate(TUR22_SULYOZOTT = eval_tidy(parse_expr(expression2[num])) / 100 * TUR22 * VGMA001_SULY) -> INN_DT
    INN_DT %>% group_by(M065_RETEG1, M0581_2J) %>% summarise(TUR22_SULYOZOTT = sum(TUR22_SULYOZOTT)) %>% arrange(M065_RETEG1, M0581_2J) -> INN_Ordered
...
for(num in 1:length(expression)){
  ...
  if(nrow(NINN_DT) != 0){
    ...
    NINN_DT %>% mutate(TUR22_SULYOZOTT = eval_tidy(parse_expr(expression2[num])) / 100 * TUR22 * VGMA001_SULY) -> NINN_DT
    NINN_DT %>% group_by(M065_RETEG1, M0581_2J) %>% summarise(TUR22_SULYOZOTT = sum(TUR22_SULYOZOTT)) %>% arrange(M065_RETEG1, M0581_2J) -> NINN_Ordered
    CIS_INDICATOR <- expression2[num]

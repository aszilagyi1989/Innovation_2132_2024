...
for(num in 1:length(expression)){
  ...
  if(nrow(INN_DT) != 0){
    ...
    INN_DT %>% mutate(TUR22_SULYOZOTT = eval_tidy(parse_expr(expression3[num])) / 100 * TUR22 * VGMA001_SULY) -> INN_DT
...
for(num in 1:length(expression)){
  ...
  if(nrow(NINN_DT) != 0){
    ...
    NINN_DT %>% mutate(TUR22_SULYOZOTT = eval_tidy(parse_expr(expression3[num])) / 100 * TUR22 * VGMA001_SULY) -> NINN_DT
